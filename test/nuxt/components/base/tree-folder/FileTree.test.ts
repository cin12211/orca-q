...
describe('FileTree', () => {
  ...
  it('auto-expands all folders when searchQuery is provided and restores on clear', async () => {
    const saveMock = vi.fn();
    const persistenceExtension = {
      loadExpandedIds: () => ['root'],
      saveExpandedIds: saveMock,
    };

    const wrapper = mountTree({
      initExpandedIds: ['root'],
    ...
    expect(wrapper.text()).toContain('root');
    expect(wrapper.text()).toContain('src');
    // Before search, 'src' folder is collapsed, so 'index.ts' is not rendered
    expect(wrapper.text()).not.toContain('index.ts');

    // Keystroke 1: user types 'ind'
    await wrapper.setProps({ searchQuery: 'ind' });
    await wrapper.vm.$nextTick();

    expect((wrapper.vm as any).isExpandedAll).toBe(true);
    expect(wrapper.text()).toContain('index.ts');

    // Keystroke 2: user types 'index' (intermediate keystroke should preserve original snapshot)
    await wrapper.setProps({ searchQuery: 'index' });
    await wrapper.vm.$nextTick();

    expect((wrapper.vm as any).isExpandedAll).toBe(true);
    expect(wrapper.text()).toContain('index.ts');

    // Clear searchQuery -> restores exact previous expansion ('root' only, 'src' collapsed)
    ...
    // Ensure persistence was restored with original state ('root' only)
    const lastSaveCall = saveMock.mock.calls[saveMock.mock.calls.length - 1];
    expect(lastSaveCall?.[0]).toEqual(['root']);
  });

  it('does not auto-expand when autoExpandOnSearch is false', async () => {
    const wrapper = mountTree({
      initExpandedIds: ['root'],
      autoExpandOnSearch: false,
      storageKey: 'test-no-auto-expand',
    });
