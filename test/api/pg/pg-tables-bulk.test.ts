...
describe('Tables Bulk Operations API — PostgreSQL', async () => {
  await setup();

  const SCHEMA = 'public';

  // Use a temporary table for mutation tests to avoid polluting fixture data.
  // Each test creates/drops its own temp table.
    ...
    it('deletes rows by primary key values', async () => {
      ...
      try {
        ...
        const verify = await $fetch('/api/query/raw-execute', {
          ...
          body: {
            ...pgStringBody(),
            query: `SELECT COUNT(*)::int AS cnt FROM ${SCHEMA}.${TABLE}`,
            params: [],
          },
        });
