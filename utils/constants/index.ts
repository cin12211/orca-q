export * from './operatorSets';

export const DEFAULT_QUERY = 'SELECT * from';
export const DEFAULT_QUERY_COUNT = 'SELECT COUNT(*) as count from';
export const DEFAULT_QUERY_SIZE = 100;

export const DEFAULT_MAX_KEEP_ALIVE = 10;
export const DEFAULT_DEBOUNCE_RANGE_SELECTION = 16;
export const DEFAULT_DEBOUNCE_INPUT = 200;
export const DEFAULT_DEBOUNCE_SCROLL = 100;
export const ONE_SECOND = 1000;
export const DEFAULT_WINDOW_NAME = 'OrcaQ';
export const DEFAULT_BUFFER_ROWS = 10;
export const DEFAULT_ROW_HEIGHT = 25;
