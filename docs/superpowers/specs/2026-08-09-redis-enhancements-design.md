...
- `pages/[workspaceId]/[connectionId]/redis/[tabViewId].vue` currently has **no** `definePageMeta({ keepalive })`, unlike every sibling tab-route page (`quick-query/[tabViewId].vue`, `agent/[tabViewId].vue` use `keepalive: { max: DEFAULT_MAX_KEEP_ALIVE }`; `erd/index.vue` etc. use `keepalive: true`). This means switching to a different top-level tab and back today fully destroys and remounts `RedisWorkspace.vue`, discarding all of its state — not just key detail. Add `definePageMeta({ keepalive: { max: DEFAULT_MAX_KEEP_ALIVE } })` (import from `~/core/constants`) to match the `quick-query`/`agent` pattern, since Redis also supports multiple concurrent tabs (one per connection). This is complementary to, not a replacement for, the in-memory cache above: it preserves state across _tab_ switches (route changes), while the cache Map handles switching _keys within_ an already-open tab (no route change, so page-level `keepalive` never applies there).
