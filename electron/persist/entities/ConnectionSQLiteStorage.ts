...
class ConnectionSQLiteStorage extends SQLite3Storage<Connection> {
  ...
  toRow(c: Connection): Record<string, unknown> {
    return {
      ...
      type: c.type,
      method: c.method,
      providerKind: c.providerKind ?? null,
      ...
      managedSqlite: c.managedSqlite ? JSON.stringify(c.managedSqlite) : null,
  ...
  fromRow(row: Record<string, unknown>): Connection {
    ...
    return {
      ...
      providerKind: r.providerKind as Connection['providerKind'],
      ...
      managedSqlite: r.managedSqlite ? JSON.parse(r.managedSqlite) : undefined,
