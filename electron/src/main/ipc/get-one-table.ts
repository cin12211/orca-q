...
export interface GetOneTableRequest extends IpcBaseRequest {
  tableName: string
}

export type DBGetOneTableResponse = IpcBaseResponse<TableMetadata>

ipcMain.handle(
  'db:get-one-table',
  async (
    _,
    { dbConnectionString, tableName }: GetOneTableRequest
  ): Promise<DBGetOneTableResponse> => {
    try {
    ...
    `)) as { table_detail: any[] }[]
      ...
      const data = result?.[0]?.table_detail?.[0]
