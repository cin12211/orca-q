...
function buildRedisUrl() {
  ...
  const port = parseInteger(readEnv('ORCAQ_REDIS_PORT', 'REDIS_PORT'), 6379, 1);
  ...
  const explicitUrl = readEnv('ORCAQ_REDIS_URL', 'REDIS_URL');
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Redis sample data ready: ${summary.keyCount} keys seeded in DB ${readEnv('ORCAQ_REDIS_DATABASE', 'REDIS_DATABASE') || '0'}.`
    );
  } finally {
    await client.quit();
  }
