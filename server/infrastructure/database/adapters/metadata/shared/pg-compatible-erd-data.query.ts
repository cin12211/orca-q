/**
 * ERD query for Postgres-wire-compatible dialects that don't implement
 * `pg_relation_size()` (Redshift never has; verified missing on CockroachDB
 * v24.2) or that expose extra internal virtual schemas alongside the user's
 * schemas (CockroachDB's `crdb_internal`/`pg_extension`). Shared by Redshift
 * and CockroachDB rather than duplicated per class since the underlying gap
 * — not the adapter's behavior — is identical for both.
 */
export const getPgCompatibleErdDataQuery = `
        WITH
            fk_info AS (
                SELECT
                ...
                WHERE
                    c.contype = 'f'
                    AND n.nspname NOT IN ('information_schema', 'pg_catalog', 'crdb_internal', 'pg_extension')
                GROUP BY
                    n.nspname,
                    cl.relname
            ),
            ...
            indexes_cols AS (
                ...
                WHERE
                    tnsp.nspname NOT IN ('information_schema', 'pg_catalog', 'crdb_internal', 'pg_extension')
                    AND tnsp.nspname NOT LIKE 'pg_%'
                GROUP BY
                    tnsp.nspname,
                    trel.relname
            ),
            columns_info AS (
                ...
                WHERE
                    cols.table_schema NOT IN ('information_schema', 'pg_catalog', 'crdb_internal', 'pg_extension')
                GROUP BY
                    cols.table_schema,
                    cols.table_name
            ),
            tbls AS (
                ...
                WHERE
                    tbls.table_schema NOT IN ('information_schema', 'pg_catalog', 'crdb_internal', 'pg_extension')
            ),
            views AS (
                SELECT
                    json_agg(
                ...
                FROM
                    pg_views views
                WHERE
                    views.schemaname NOT IN ('information_schema', 'pg_catalog', 'crdb_internal', 'pg_extension')
            ),
            config AS (
                SELECT
                    json_agg(
